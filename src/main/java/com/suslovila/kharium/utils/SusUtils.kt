...
object SusUtils {
    ...
    fun getOrthogonalVec3(vec3: SusVec3): SusVec3 {
        ...
        when (notNullVec3Cords.size) {
            ...
            2 -> {
                with(notNullVec3Cords) {
                    ...
                    orthogonal = SusVec3.getVectorFromArrayList(
                        arrayListOf(0.0, 0.0, 0.0).setAndGet(
                            this[1].first,
                            this[0].second
                        ).setAndGet(this[0].first, -this[1].second)
                    )
                }
            }
            ...
            1 -> orthogonal = SusVec3.getVectorFromArrayList(
                arrayListOf(0.0, 0.0, 0.0).setAndGet(
                    (notNullVec3Cords[0].first + 1) % 3,
                    notNullVec3Cords[0].second
                )
            )
    ...
    fun getIndicesCycledFrom(index: Int, maxSize: Int): LinkedList<Int> {
        val listWithIndexes = LinkedList<Int>()
        for (j in index until maxSize) {
...
class Percentage(
    ...
    init {
        value = valueIn.coerceIn(0.0, 1.0)
    }
}

operator fun Percentage.plusAssign(percentage: Percentage) {
    this.value += percentage.value
}

operator fun Percentage.minusAssign(percentage: Percentage) {
    this.value -= percentage.value
}
