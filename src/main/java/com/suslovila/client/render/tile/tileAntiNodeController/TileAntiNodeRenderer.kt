...
import com.suslovila.utils.SusUtils.antiNodeAppearanceTime
import com.suslovila.utils.SusVec3
import com.suslovila.utils.WorldHelper.getPos
...
object TileAntiNodeRenderer : SusTileRenderer<TileAntiNode>() {
    ...
    override fun render(antiNode: TileAntiNode, partialTicks: Float) {
        ...
        if (distance <= viewDistance) {
            renderAntiNode(
                viewer, viewDistance, false, 1.0f, antiNode.xCoord, antiNode.yCoord, antiNode.zCoord, partialTicks, 0.5f * Math.min(
                    antiNode.tickExisted.toFloat() / 120, 1f
                )
            )
            if (antiNode.tickExisted > antiNodeAppearanceTime / 2) spawnShadowParticlesOutPutVersion(
                antiNode,
                viewer.worldObj,
                5,
                2,
            ...
            if (!Minecraft.getMinecraft().isGamePaused) {
                if (random.nextInt(9) == 6) {
                    ...
                    ExampleMod.proxy.nodeAntiBolt(
                        viewer.worldObj,
                        antiNode.xCoord + 0.5f,
                        antiNode.yCoord + 0.5f,
                        antiNode.zCoord + 0.5f,
                        (antiNode.xCoord + nextDouble(-lightningLength.toDouble(), lightningLength.toDouble())).toFloat(),
                        (antiNode.yCoord + nextDouble(-lightningLength.toDouble(), lightningLength.toDouble())).toFloat(),
                        (antiNode.zCoord + nextDouble(-lightningLength.toDouble(), lightningLength.toDouble())).toFloat()
                    )
                }
            }
        }
    ...
    fun spawnShadowParticlesOutPutVersion(
        antiNode: TileAntiNode,
        world: World,
        maxTailsAmount: Int,
        minTailSpeed: Int,
        maxTailSpeed: Int,
        wholeIterationAmount: Int,
        minRadius: Double,
        maxRadius: Double,
            ...
            if (random.nextInt(50) == 37 && antiNode.kharuTails.size < maxTailsAmount) {
                antiNode.kharuTails.add(
                    KharuTail(
                        ...
                        aimVec3 = SusVec3(nextDouble(-1.0, 1.0), nextDouble(-1.0, 1.0), nextDouble(-1.0, 1.0)).normalize()
                    )
                )
            }
        }
        antiNode.kharuTails.forEach { kharuTail ->
            ...
            while (h < kharuTail.tailSpeed && antiNode.kharuTails.contains(kharuTail)) {
                ...
                val offsetFromHome = m.scale(Math.sin(kharuTail.timer * Math.PI / 90)).add(k.scale(Math.cos(kharuTail.timer * Math.PI / 90)))
                kharuTail.timer++
                val particlePosition = kharuTail.homePos.add(offsetFromHome)
                val particle = FXAntiNode(
                    world,
                    particlePosition.x,
                    particlePosition.y,
                    particlePosition.z,
